...
router.post('/freelancer_bids', auth, async (req, res) => {
  try {
    const { skill, bidNumber } = req.body;
    ...
    await pool.query(
      'INSERT INTO freelancer_bids (id, skill, bid_number, created_at, updated_at, user_id) VALUES (?, ?, ?, ?, ?, ?)',
      [id, skill, bidNumber, now, now, userId]
    );
...
router.put('/freelancer_bids/:id', auth, async (req, res) => {
  try {
    ...
    await pool.query(
      'UPDATE freelancer_bids SET skill = ?, bid_number = ?, updated_at = ? WHERE id = ?',
      [skill, bidNumber, now, id]
    );
...
router.post('/freelancer_chat', auth, async (req, res) => {
  try {
    const { clientName, clientCountry, projectTitle, review, reviewNumber, spentMoney, isAwarded } = req.body;
    const userId = req.user.id; // Get user ID from auth token
    ...
    await pool.query(
      'INSERT INTO freelancer_chats (id, client_name, client_country, project_title, review, review_number, spent_money, is_awarded, user_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [id, clientName, clientCountry, projectTitle, review, reviewNumber, spentMoney, isAwarded, userId, now, now]
    );
    
    const [newChat] = await pool.query('SELECT * FROM freelancer_chats WHERE id = ?', [id]);
    res.status(201).json({
      id: newChat[0].id,
...
router.put('/freelancer_chat/:id', auth, async (req, res) => {
  try {
    ...
    await pool.query(
      'UPDATE freelancer_chats SET client_name = ?, client_country = ?, project_title = ?, review = ?, review_number = ?, spent_money = ?, is_awarded = ? WHERE id = ?',
      [clientName, clientCountry, projectTitle, review, reviewNumber, spentMoney, isAwarded, id]
    );
